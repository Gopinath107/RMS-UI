...
export default function SystemAdminSidebar({ 
  ...
  // Get total users count and interview assignments
  useEffect(() => {
    const updateCounts = () => {
      try {
  ...
  return (
    ...
    >
      ...
      <div className={`${isExpanded ? 'p-4' : 'p-3'} border-b border-red-300/20`}>
        <div className="flex items-center gap-3">
          <motion.button
            onClick={() => setIsExpanded(!isExpanded)}
            className="w-12 h-12 bg-gradient-to-r from-red-400 to-rose-500 rounded-xl flex items-center justify-center hover:from-red-500 hover:to-rose-600 transition-all duration-200 shadow-lg flex-shrink-0"
          ...
          {isExpanded && (
            ...
            >
              ...
              <div className="flex flex-col justify-center min-w-0">
                ...
                <h2 className="text-red-200 font-medium text-sm leading-tight">
                  Control Center
                </h2>
              </div>
            </motion.div>
              ...
              >
                <motion.button
                  ...
                  className={`w-full flex items-center ${
                    ...
                    isActive 
                      ? 'bg-gradient-to-r from-red-500/30 to-rose-500/30 text-white border border-red-400/30 shadow-lg'
                ...
                >
                  ...
                  {isExpanded && (
                    <>
                      ...
                      >
                        ...
                        <p className="text-xs text-red-300 truncate">{item.description}</p>
                      </motion.div>
                      {/* Show notification badge for user count */}
                      {item.badge && (
                        <motion.div
                          initial={{ scale: 0 }}
                          animate={{ scale: 1 }}
                  ...
                  {isActive && !isExpanded && (
                    <motion.div
                      ...
                      className="absolute right-1 w-1 h-8 bg-red-300 rounded-full"
                    />
                  )}
      ...
      {/* User Info & Logout */}
      <div className={`mt-auto p-3 ${isExpanded ? 'px-3' : 'px-2'} border-t border-red-300/20 space-y-3`}>
        {isExpanded && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.3 }}
            className="bg-red-500/10 rounded-lg p-3 border border-red-400/20"
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-white font-medium text-sm truncate">
                  {localStorage.getItem('userName') || 'System Admin'}
                </p>
                <p className="text-red-300 text-xs truncate">Administrator</p>
              </div>
            </div>
          </motion.div>
        )}
