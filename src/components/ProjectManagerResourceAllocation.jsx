...
const InterviewResultsTable = memo(({ 
  ...
  const interviewResultColumns = [
    ...
    },
    {
      ...
      render: (result) => (
        <div className="flex flex-col gap-1">
          <Badge className={`text-xs ${result.requestType === 'OPP' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}>
            {result.requestType}
          </Badge>
          {result.requestType === 'DR' && result.demandId && <span className="text-xs text-gray-600">Demand: {result.demandId}</span>}
          {result.groupId && <span className="text-xs text-gray-600">Group: {result.groupId}</span>}
        </div>
      ),
    },
    {
      ...
      render: (result) => (
        <div className="text-xs sm:text-sm whitespace-nowrap">
          <div>{result.employeeName} {result.candidateName}</div>
          <div className="text-gray-500">{result.candidateEmail}</div>
        </div>
      ),
    },
    {
      ...
      render: (result) => (
        <div className="flex flex-col gap-1">
          <Badge className={`text-xs ${result.resourceType === 'Internal' ? 'bg-blue-100 text-blue-800 border border-blue-200' : 'bg-purple-100 text-purple-800 border border-purple-200'}`}>
            {result.resourceType}
          </Badge>
          {result.resourceId && result.resourceId !== 'N/A' && <span className="text-xs text-gray-600 font-mono">ID: {result.resourceId}</span>}
        </div>
      ),
    },
    {
    ...
    { key: 'requestedDate', label: 'Requested Date', render: (result) => result.requestedDate ? formatDateTime(result.requestedDate, 'date') : <span className="text-gray-400">-</span> },
    { key: 'fulfilmentDate', label: 'Fulfilment Date', render: (result) => result.fulfilmentDate ? formatDateTime(result.fulfilmentDate, 'date') : <span className="text-gray-400">-</span> },
    {
      key: 'status',
      label: 'Status',
      type: 'status',
      render: (result) => (
        <div className="space-y-1">
          <Badge className={`${getStatusBadgeColor(result.status)} text-xs sm:text-sm`}>{result.status}</Badge>
          {result.status === 'Cancelled' && result.cancellationReason && (
            <div className="text-xs text-red-600 truncate" title={result.cancellationReason}>Reason: {result.cancellationReason}</div>
          )}
        </div>
      ),
    },
    {
      ...
      render: (result) => (
        <div className="flex items-center gap-1">
          ...
          {result.clearanceStatus === 'Cleared' && <Award className="w-3 h-3 sm:w-4 sm:h-4 text-green-600" />}
        </div>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (result) => (
        <div className="flex items-center gap-2 flex-wrap">
          {result.clearanceStatus === 'Cleared' && result.status === 'Selected' && result.onboardingStatus === 'Onboarded' && !result.allocatedToClient && (
            <Button size="sm" onClick={() => handleAllocateToClient(result)} className="h-6 sm:h-8 px-2 sm:px-3 text-xs sm:text-sm bg-green-600 hover:bg-green-700 text-white">
              <UserCheck className="w-3 h-3 sm:w-4 sm:h-4 mr-1" />
              Allocate
            </Button>
          )}
          {result.allocatedToClient && <Badge className="bg-blue-100 text-blue-800 text-xs sm:text-sm font-medium">Allocated</Badge>}
          {result.clearanceStatus === 'Cleared' && result.status === 'Selected' && result.onboardingStatus !== 'Onboarded' && !result.allocatedToClient && (
            <Badge className="bg-orange-100 text-orange-800 text-xs sm:text-sm">Awaiting Onboarding</Badge>
          )}
          {result.status === 'Selected' && result.clearanceStatus === 'Pending' && (
            <Badge className="bg-orange-100 text-orange-800 text-xs sm:text-sm">Awaiting HR Update</Badge>
          )}
        </div>
      ),
    },
    ...
    <Card className="shadow-md bg-white/95 backdrop-blur-sm border border-gray-200">
      ...
      <CardContent>
        <div className="relative">
          <ReusableDataView
            tableKey="pm-interview-results-table"
            data={paginatedResults}
            columns={interviewResultColumns}
            rowKey="id"
            emptyMessage="No interview results found. Interviews scheduled in HR management will appear here."
            defaultViewMode="table"
            pagination={{
              currentPage: currentPageResults,
              totalPages: totalPagesResults,
              onPageChange: setCurrentPageResults,
              pageSize: rowsPerPageResults,
              onPageSizeChange: (value) => {
                setRowsPerPageResults(value);
                setCurrentPageResults(1);
              },
              pageSizeOptions: [5, 10, 20, 50],
              totalItems: filteredResults.length,
            }}
            tableClassName="table-layout-auto"
            tableHeaderClassName="bg-green-50 hover:bg-green-100"
            tableContainerClassName="max-h-[70vh]"
          />
          {false && <div className="overflow-x-auto max-h-[60vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
