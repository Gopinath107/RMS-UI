  ...
  const interviewColumns = [
    {
      ...
      type: 'status',
      render: (i) => (
        <Badge className={i.interviewType === 'OPP' ? 'bg-purple-100 text-purple-800' : 'bg-indigo-100 text-indigo-800'}>
          {i.interviewType}
        </Badge>
      ),
    },
    {
      key: 'submittedDate',
      label: 'Created at',
      render: (i) => i.submittedDate ? new Date(i.submittedDate).toLocaleDateString() : 'N/A',
    },
    { key: 'requestId', label: 'Request ID', render: (i) => i.requestId },
    {
      key: 'candidate',
      label: 'Candidate',
      render: (i) => (
    ...
    },
    {
      ...
      type: 'status',
      render: (i) => <Badge className={getLevelBadgeColor(i.overallStatus)}>{i.overallStatus}</Badge>,
    },
    {
      key: 'resourceType',
      label: 'Resources',
    ...
    },
    {
      ...
      render: (i) => (
        <div className="flex space-x-2">
          <Button variant="ghost" size="sm" onClick={() => handleViewInterview(i)} className="text-blue-600 hover:text-blue-700">
            <FileText className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => handleEditInterview(i)} className="text-yellow-600 hover:text-yellow-700">
            <Pencil className="h-4 w-4" />
          </Button>
          <Button variant="ghost" size="sm" onClick={() => handleDeleteClick(i)} className="text-red-600 hover:text-red-700">
    ...
    <SelectContent>
        ...
        <SelectItem value="external">External</SelectItem>
          ...
          <ReusableDataView
            ...
            tableContainerClassName="max-h-[520px]"
          />
          {false && (paginated.length > 0 ? (
                  ...
                  <TableBody>
                    {paginated.map((i) => (
                      <TableRow key={i.id}>
                        ...
                        <TableCell>{i.requestId}</TableCell>
...
<Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
  <DialogContent className="sm:max-w-[600px] w-[95vw] max-w-[95vw] max-h-[90vh] flex flex-col p-0 overflow-hidden">
    ...
    <div className="flex-1 overflow-y-auto p-6 space-y-4">
      ...
      {!isEditMode && formData.interviewType === 'demand' && (
        <div data-error={!!formErrors.demandId}>
          ...
          >
            <SelectTrigger className={`border-gray-300 focus:ring-blue-500 ${
              formErrors.demandId ? 'border-red-500' : ''
            }`}>
            ...
            <SelectContent className="max-h-40 overflow-y-auto">
              {allDemands.filter(d => d && (d.demandId || d.demandid)).map((d) => (
                <SelectItem key={(d.demandId || d.demandid)} value={(d.demandId || d.demandid).toString()}>
                  {`DM-${(d.demandId || d.demandid).toString().padStart(3, '0')} - ${(d.demandTitle || d.demandtitle)} (${d.clientName || 'Unknown'})`}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
      ...
      <div>
        ...
        <Select
          ...
          disabled={isEditMode || formData.interviewType === 'demand'}
        >
          <SelectTrigger className="border-gray-300 focus:ring-blue-500">
      ...
      <div data-error={!!formErrors.requestId}>
        ...
        <Select
          ...
          disabled={isEditMode || (formData.interviewType === 'demand' && !formData.demandId)}
        >
          <SelectTrigger className={`border-gray-300 focus:ring-blue-500 ${
            formErrors.requestId ? 'border-red-500' : ''
          }`}>
            <SelectValue placeholder="Select request" />
          </SelectTrigger>
          <SelectContent className="max-h-40 overflow-y-auto">
            {formData.interviewType === 'demand' 
              ? (demandResourceRequests || []).map((r) => {
                  if (!r || !r.requestId) return null;
                  return (
                    <SelectItem key={r.requestId} value={r.requestId.toString()}>
                      {`REQ-${r.requestId.toString().padStart(3, '0')} - ${r.candidateName || 'Unknown'} (${r.resourceType || 'Unknown'})`}
                    </SelectItem>
                  );
                })
              : activeOpportunityRequests.map((r) => (
                  <SelectItem key={r.requestId} value={r.requestId.toString()}>
                    {`REQ-${r.requestId.toString().padStart(3, '0')} - ${r.groupTitle || r.projectName}`}
                  </SelectItem>
                ))
            }
          </SelectContent>
...
<div data-error={!!formErrors.candidateId}>
  ...
  {isEditMode || formData.interviewType === 'demand' ? (
    ...
    <>
      {isLoadingExternalResources && formData.resourceType === 'external' ? (
        ...
        >
          <SelectTrigger className={`border-gray-300 focus:ring-blue-500 ${
            formErrors.candidateId ? 'border-red-500' : ''
          }`}>
  ...
  {!isEditMode && formData.resourceType === 'external' && externalResources.length === 0 && !isLoadingExternalResources && (
    <p className="text-yellow-600 text-sm mt-1 flex items-center">
      ...
      {/* Interview Levels */}
      <div data-error={!!formErrors.interviewLevels}>
    ...
    >
      ...
      {formData.interviewType === 'opportunity' && (
        <div>
          ...
          >
            <SelectTrigger className={`border-gray-300 focus:ring-blue-500 ${
              levelErrors.interviewer ? 'border-red-500' : ''
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <Input
            ...
            className={`border-gray-300 focus:ring-blue-500 ${
              levelErrors.date ? 'border-red-500' : ''
            } ${isDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
            disabled={isDisabled}
          />
          {levelErrors.date && (
        ...
        <div>
          ...
          <Input
            ...
            className={`border-gray-300 focus:ring-blue-500 ${
              levelErrors.time ? 'border-red-500' : ''
            } ${isDisabled ? 'bg-gray-100 cursor-not-allowed' : ''}`}
            disabled={isDisabled}
          />
          {levelErrors.time && (
