...
function renderSortIcon(column, sortState) {
  ...
  if (active !== key) return <ArrowUpDown className="h-3 w-3 opacity-40 shrink-0" />;
  return dir === "desc"
...
function CustomizeColumnsPopover({ open, onClose, columns, preferences, onApply, onReset, trigger }) {
  ...
  return (
    <Popover open={open} onOpenChange={(v) => !v && onClose()}>
      ...
      <PopoverContent align="end" className="w-[320px] p-0 rounded-xl shadow-2xl">
        ...
        <div className="px-4 py-3 border-t border-gray-100 flex items-center justify-between gap-2 bg-gray-50/50 rounded-b-xl">
          <Button type="button" variant="ghost" size="sm" onClick={() => { onReset(); onClose(); }} className="text-gray-500 hover:text-gray-700 h-8 px-2 text-xs">
            Reset
          </Button>
...
export function DataViewToolbar({ controls, className = "" }) {
  ...
  const btnBase =
    "inline-flex items-center justify-center rounded-md border border-gray-200 bg-white p-2 text-gray-700 shadow-sm hover:bg-gray-50 transition-colors";
...
export default function ReusableDataView({
  ...
  const showInternalToolbar = !externalControls && !renderToolbar;

  return (
    <div className="space-y-0">
      {/* Custom rendered toolbar */}
      {renderToolbar && renderToolbar(<DataViewToolbar controls={controls} />)}

      {/* Internal toolbar — only shown when no external controls and no custom renderToolbar provided */}
      {showInternalToolbar && (
