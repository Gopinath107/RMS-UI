...
const RequestResource = ({ setCurrentPage }) => {
  ...
  const requestColumns = [
    { key: "projectName", label: "Project", render: (request) => <span className="font-semibold">{request.projectName}</span> },
    { key: "reqId", label: "Req ID", render: (request) => request.reqId },
    { key: "numberOfResources", label: "Resources", render: (request) => request.numberOfResources },
    { key: "locationType", label: "Location Type", render: (request) => request.locationType },
    { key: "workMode", label: "Work Mode", render: (request) => request.workMode },
    { key: "location", label: "Location", render: (request) => request.location },
    { key: "priority", label: "Priority", type: "status", render: (request) => <Badge className={getPriorityBadgeColor(request.priority)}>{request.priority}</Badge> },
    { key: "status", label: "Status", type: "status", render: (request) => <Badge className={getStatusBadgeColor(request.status)}>{request.status}</Badge> },
    { key: "submittedDate", label: "Submitted", render: (request) => request.submittedDate },
    { key: "estimatedCost", label: "Estimated Cost", render: (request) => request.estimatedCost },
    {
      key: "skills",
      label: "Skills",
      render: (request) => (
        <div className="flex flex-wrap gap-1">
          ...
          {request.skills?.length > 3 && <Badge variant="outline" className="text-xs">+{request.skills.length - 3} more</Badge>}
        </div>
      ),
    },
  ];
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      {/* Requests Grid */}
      <ReusableDataView
        tableKey="resource-requests-table"
