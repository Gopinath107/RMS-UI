...
export default function Timesheets({ setCurrentPage }) {
  ...
  const timesheetColumns = [
    ...
    { key: "clientName", label: "Client", render: (timesheet) => timesheet.clientName },
    { key: "week", label: "Week", render: (timesheet) => `${new Date(timesheet.startDate).toLocaleDateString()} - ${new Date(timesheet.endDate).toLocaleDateString()}` },
    { key: "monday", label: "Mon", render: (timesheet) => `${timesheet.hours.monday}h` },
    { key: "tuesday", label: "Tue", render: (timesheet) => `${timesheet.hours.tuesday}h` },
    { key: "wednesday", label: "Wed", render: (timesheet) => `${timesheet.hours.wednesday}h` },
    { key: "thursday", label: "Thu", render: (timesheet) => `${timesheet.hours.thursday}h` },
    { key: "friday", label: "Fri", render: (timesheet) => `${timesheet.hours.friday}h` },
    { key: "totalHours", label: "Total", render: (timesheet) => <span className="font-bold text-purple-700">{timesheet.totalHours}h</span> },
    {
      key: "status",
      label: "Status",
      type: "status",
      ...
      render: (timesheet) => (
        <div className="flex flex-wrap gap-2">
          {timesheet.status === "draft" && (
            <Button size="sm" onClick={() => handleSubmitTimesheet(timesheet.id)} className="bg-blue-500 hover:bg-blue-600">
              <Send className="w-3 h-3 mr-1" />
              Submit
            </Button>
          )}
          <Button size="sm" variant="outline" onClick={() => handleSendEmail(timesheet)} className="hover:bg-green-50">
            <Send className="w-3 h-3 mr-1" />
            Email
          </Button>
          <Button size="sm" variant="outline" onClick={() => deleteTimesheet(timesheet.id)} className="hover:bg-red-50 text-red-600">
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Pagination */}
      {false && <div className="flex items-center justify-between mt-6">
