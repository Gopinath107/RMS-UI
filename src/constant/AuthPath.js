...
const isDevelopment = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' || window.location.hostname === '[::1]';
const isAWS = window.location.hostname === '98.130.57.74';

export const APIConfigurations = {
 
  rootURL: (isDevelopment || window.location.port === '5173' || window.location.port === '5174')
    ? "/api"                             // Use Vite proxy in dev to avoid CORS
    : isAWS
